...
class MacroArguments extends Component {
    constructor(props) {
        super(props);
        this.state = {
            macroArguments: this.props.macroArguments,
            newAdded: false,
            editIndex: -1,
            deleteIndex: -1,
    ...
    getmacroAgruments = () => {
        const thisObj = this;
        CallServerPost('MacroTemplate/GetMacroArguments', {})
            .then(
                function (response) {
                    if (response.status == 1) {
                        ...
                        for (var i = 0; i < thisObj.state.macroArguments.length; i++) {
                            thisObj.state.selectedArgs.push(thisObj.state.macroArguments[i].name)
                        }
    ...
    confirmDelete = (k) => {
        ...
        uuid--;
        var macroArguments = [];
        for (var i = 0; i < this.state.macroArguments.length; i++) {
            if (i !== k) {
                macroArguments.push(this.state.macroArguments[i]);
            }
        }
        this.setState({ popvisible: false, macroArguments: macroArguments });
        // this.props.form.resetFields();
    }
        ...
        thisObj.props.form.validateFields(["argumentNameItem" + k, "typeItem" + k, "descriptionItem" +k], { force: true }, (err, values) => {
            if (!err) {
                if (this.state.editIndex > -1) {
                    ...
                    this.state.macroArguments[editIndexNow].name = values["argumentNameItem" + editIndexNow];
                    this.state.macroArguments[editIndexNow].type = values["typeItem" + editIndexNow];
                    this.state.macroArguments[editIndexNow].desc = values["descriptionItem" + editIndexNow];
                } else {
                    var editIndexNow = this.state.macroArguments.length;
                    this.state.macroArguments.push({ name: values["argumentNameItem" + editIndexNow], type: values["typeItem" + editIndexNow], desc: values["descriptionItem" + editIndexNow] });
                }
    ...
    handleCancel = () => {
        this.reloadModal();
        this.props.handleCancel(this.state.macroArguments);
    }

    reloadModal = () => {
        if (this.state.macroArguments.length === keys.length) {
            uuid = 0;
            keys = [];
        }
        
        // this.setState({ macroArguments: null, editIndex: -1, newAdded: false, deleteIndex: -1 });
    ...
    fnToValidateArgumentName = (rule, value, callback) => {
        ...
        for (var i = 0; i < thisObj.state.macroArguments.length; i++) {
            if (thisObj.state.macroArguments[i].name === value && document.getElementById(rule.field).className.includes('ant-select-enabled') && thisObj.state.editIndex !== i) {
    ...
    render() {
        ...
        const { editIndex, newAdded, descDataSource, nameDataSource, macroArguments, selectedFlag, selectedArgs } = this.state;
       
        const filteredNameDataSrcs = () => {
            const tempArr = [...nameDataSource];
            for (var i = 0; i < macroArguments.length; i++) {
                for (var j = 0; j < tempArr.length; j++) {
                    if (tempArr[j] === macroArguments[i].name) {
                        tempArr.splice(j, 1);
                    }
                }
            }
        ...
        const formItems = macroArguments != null && keys.map((k, index) => {
            return (
                <Row gutter={16} key={k}>
                    <Col key={'argumentCol' + k} span={6}>
                        ...
                        >
                            {getFieldDecorator('argumentNameItem' + k, {
                                ...
                                initialValue: k < macroArguments.length ? macroArguments[k].name : "",
                            })(
                                <AutoComplete
                                    dataSource={selectedFlag === true ? filteredNameDataSrcs() : []}
                                    tabIndex="0"
                                    placeholder="Argument Name"
                                    disabled={k < macroArguments.length && k != editIndex}
                    ...
                    <Col key={'typeCol' + k} span={8}>
                        ...
                        >
                            {getFieldDecorator('typeItem' + k, {
                                ...
                                initialValue: k < macroArguments.length ? macroArguments[k].type : "",
                            })(
                                <Select tabIndex="0" disabled={(k < macroArguments.length && k != editIndex) || getFieldValue('typeItem' + k) == "--Select--"} placeholder="Select Type" key={'typeSelect' + k}>
                                    {
                    ...
                    <Col key={'rolesCol' + k} span={8}>
                        ...
                        >
                            {getFieldDecorator('descriptionItem' + k, {
                                ...
                                initialValue: k < macroArguments.length ? macroArguments[k].desc : "",
                            })(
                                <AutoComplete
                    ...
                    </Col>
                    {
                        ...
                        this.props.viewOnly ? "" : k < macroArguments.length && k != editIndex ?
                            (<Col>
...
const WrappedApp = Form.create()(MacroArguments);
