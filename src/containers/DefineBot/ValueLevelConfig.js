...
class ValueLevelConfig extends Component {
    ...
    /////we have to arrange the selected checkbox text in the input field based on the the sibilngs domain in the tree
    //    let temp1 = [e[0]];

    //    e.forEach(function (key, index) {
    //        thisObj.props.sibling.forEach(function (siblingKey, siblingIndex) {
    //            if (index !== 0) {
    //                if (key === siblingKey.Title) {
    //                    temp1.push(key);
    //                }
    //            }
    //        });
