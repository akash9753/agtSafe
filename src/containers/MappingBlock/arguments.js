...
class MappingBlockArguments extends Component {
    constructor(props) {
        super(props);
        this.state = {
            MappingBlockArguments: this.props.pyArgs,
            newAdded: false,
            editIndex: -1,
            deleteIndex: -1,
        ...
        if (!this.props.viewOnly) {
            this.getMappingBlockArguments();
        }

    }

    getMappingBlockArguments = () => {
        const thisObj = this;
        CallServerPost('MappingBlockTemplate/GetMappingBlockArguments', {})
            .then(
                function (response) {
                    if (response.status == 1) {
                        ...
                        for (var i = 0; i < thisObj.state.MappingBlockArguments.length; i++) {
                            thisObj.state.selectedArgs.push(thisObj.state.MappingBlockArguments[i].name)
                        }
    ...
    confirmDelete = (k) => {
        ...
        uuid--;
        var MappingBlockArguments = [];
        for (var i = 0; i < this.state.MappingBlockArguments.length; i++) {
            if (i !== k) {
                MappingBlockArguments.push(this.state.MappingBlockArguments[i]);
            }
        }
        this.setState({ popvisible: false, MappingBlockArguments: MappingBlockArguments });
        // this.props.form.resetFields();
    }
        ...
        thisObj.props.form.validateFields((err, values) => {
            if (!err) {
                if (this.state.editIndex > -1) {
                    ...
                    this.state.MappingBlockArguments[editIndexNow].name = values["argumentNameItem" + editIndexNow];
                    this.state.MappingBlockArguments[editIndexNow].type = values["typeItem" + editIndexNow];
                    this.state.MappingBlockArguments[editIndexNow].desc = values["descriptionItem" + editIndexNow];
                } else {
                    var editIndexNow = this.state.MappingBlockArguments.length;
                    this.state.MappingBlockArguments.push({ name: values["argumentNameItem" + editIndexNow], type: values["typeItem" + editIndexNow], desc: values["descriptionItem" + editIndexNow] });
                }
    ...
    handleCancel = () => {
        this.reloadModal();
        this.props.handleCancel(this.state.MappingBlockArguments);
    }
    ...
    fnToValidateArgumentName = (rule, value, callback) => {
        ...
        for (var i = 0; i < thisObj.state.MappingBlockArguments.length; i++) {
            if (thisObj.state.MappingBlockArguments[i].name === value && document.getElementById(rule.field).className.includes('ant-select-enabled') && thisObj.state.editIndex !== i) {
    ...
    render() {
        ...
        const { editIndex, newAdded, descDataSource, nameDataSource, MappingBlockArguments, selectedFlag, selectedArgs } = this.state;
        //const selectedNames = getFieldValue("");
        const filteredNameDataSrcs = () => {
            const tempArr = [...nameDataSource];
            for (var i = 0; i < MappingBlockArguments.length; i++) {
                for (var j = 0; j < tempArr.length; j++) {
                    if (tempArr[j] === MappingBlockArguments[i].name) {
                        tempArr.splice(j, 1);
                    }
                }
            }
        ...
        if (MappingBlockArguments != null && uuid == 0) {
            for (var i = 0; i < MappingBlockArguments.length; i++) {
                keys.push(i);
                uuid++;
            }
        ...
        const formItems = MappingBlockArguments != null && keys.map((k, index) => {
            return (
                <Row gutter={16} key={k}>
                    <Col key={'argumentCol' + k} span={6}>
                        ...
                        >
                            {getFieldDecorator('argumentNameItem' + k, {
                                ...
                                initialValue: k < MappingBlockArguments.length ? MappingBlockArguments[k].name : "",
                            })(
                                <AutoComplete
                                    dataSource={selectedFlag === true ? filteredNameDataSrcs() : []}
                                    tabIndex="0"
                                    placeholder="Argument Name"
                                    disabled={k < MappingBlockArguments.length && k != editIndex}
                    ...
                    <Col key={'typeCol' + k} span={8}>
                        ...
                        >
                            {getFieldDecorator('typeItem' + k, {
                                ...
                                initialValue: k < MappingBlockArguments.length ? MappingBlockArguments[k].type : "",
                            })(
                                <Select tabIndex="0" disabled={(k < MappingBlockArguments.length && k != editIndex) || getFieldValue('typeItem' + k) == "--Select--"} placeholder="Select Type" key={'typeSelect' + k}>
                                    {
                    ...
                    <Col key={'rolesCol' + k} span={8}>
                        ...
                        >
                            {getFieldDecorator('descriptionItem' + k, {
                                ...
                                initialValue: k < MappingBlockArguments.length ? MappingBlockArguments[k].desc : "",
                            })(
                                <Input disabled={k < MappingBlockArguments.length && k != editIndex} />
                            )}
                    ...
                    </Col>
                    {
                        ...
                        this.props.viewOnly ? "" : k < MappingBlockArguments.length && k != editIndex ?
                            (<Col>
        ...
        return (
            ...
            <Modal
                ...
                title={"MappingBlock Template Arguments"}
                style={{ top: 20 }}
                ...
                footer={[
                    <div>
                        ...
                        {
                            MappingBlockArguments.length > 0 &&
...
const WrappedApp = Form.create()(MappingBlockArguments);
