...
class AddCDISCDataStdRole extends Component {
    ...
    getFormData = (data) => {
        if (data.action != "" && data.action != "Delete") {
            CallServerPost('CDISCDataStdRole/GetCDISCDataStdRoleFormData', { FormName: "CDISCDataStdRole", ActionName: data.action, ID: data.cDISCDataStdRoleID, Editable: (data.action === "Update") ? (data.readOnly) : false }).then(
                function (response) {
    ...
    showListPage = () => {
        this.handleCDISCDataStdRoleCancel();
        this.handleCancelEditConfirmationModal();
        this.setState({ modalLoading: false });
        this.setState({ title: "CDISC Data Standard Role", cdiscDataStdVersionID: thisObj.props.stdVersionIDForCreateAndUpdate, showCDISCDataStdRole: true })
    }
    ...
    handleSubmit = (e) => {
        ...
        if (thisObj.props.action == "Update") {
            thisObj.props.form.validateFields(["DataStdRoleName"], { force: true }, (err, values) => {
                if (!err) {
                    thisObj.setState({ showEditConfirmationModal: true, DataStdRoleName: values.DataStdRoleName });
                }
            });
        }
            ...
            thisObj.props.form.validateFields(["DataStdRoleName"], { force: true }, (err, values) => {
                if (!err) {
                    ...
                    PostCallWithZone('CDISCDataStdRole/Create', values)
                        .then(
                        function (response) {
    ...
    handleUpdate = (ChangeReason) => {
        ...
        values["CDISCDataStdRoleID"] = thisObj.props.cDISCDataStdRoleID;
        values["CDISCDataStdVersionID"] = thisObj.props.stdVersionIDForCreateAndUpdate;
        values["DataStdRoleName"] = thisObj.state.DataStdRoleName;
        ...
        thisObj.setState({ loading: true, disableBtn: true, modalLoad: true });     
                PostCallWithZone('CDISCDataStdRole/Update', values)
                    .then(
                    function (response) {
                        if (response.status == 1) {
                            thisObj.props.handleCancel();
    ...
    render() {
        ...
        return (
            <Modal
                ...
                maskClosable={false}
                onCancel={this.state.disableBtn ? null : this.handleCDISCDataStdRoleCancel}
                onOk={this.handleSubmit}
                footer={[
                    <Button key="Cancel" name="Cancel" disabled={this.state.disableBtn} className='ant-btn sc-ifAKCX fcfmNQ ant-btn-danger' style={{ float: 'left' }} onClick={this.handleCDISCDataStdRoleCancel}>
                        Cancel
                    </Button>,
                    (action == "Create") &&
            ...
            >
                ...
                <ConfirmModal title="Update Data Standard Role" SubmitButtonName="Update" onSubmit={this.handleUpdate} visible={this.state.showEditConfirmationModal} handleCancel={this.handleCancelEditConfirmationModal} getFieldDecorator={getFieldDecorator} loading={this.state.modalLoad}/>
...
const WrappedApp = Form.create()(AddCDISCDataStdRole);
